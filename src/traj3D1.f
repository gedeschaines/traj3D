...
C                            LIST OUTPUT
  ...
  355 LAT = 90.0 - THETAX*RAD
      IF (TFINAL .GT. 0.0) GO TO 356
CGED  LON = PHIX*RAD - WE*(T-TO)*RAD  ! INERTIAL TRAJ WRT To
      LON = PHIX*RAD                  ! OBSERVED TRAJ WRT T
      LONT = LON + WE*(T-TO)*RAD  ! ROTATE EARTH RELATIVE TO PHIO
      GO TO 357
  356 CONTINUE
CGED  LON = PHIX*RAD - WE*(T-T0)*RAD      ! INERTIAL TRAJ WRT To
      LON = PHIX*RAD - WE*(TFINAL-T)*RAD  ! INERTIAL TRAJ WRT Tfinal
      LONT = LON
  357 CONTINUE
      GAMMA = GAMR*RAD
      AZ = AZR*RAD
...
CGED  GRANGE = RE*(ACOS(COS(THETAX - THETO)*COS(PHIX - PHIO)))/6080.0 + RANGEO
      ...
      XLONG0 = XLONG
CGED  IF (TFINAL.GT.0.0) XLONG0 = XLONG - WE*(T-TO)*RAD       ! INERTIAL TRAJ WRT To
